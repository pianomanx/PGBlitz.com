...
initial () {
  ...
  if [ "$boxversion" == "official" ]; then ansible-playbook /opt/pgblitz/menu/pgbox/pgboxcommunity.yml
  else ansible-playbook /opt/pgblitz/menu/pgbox/pgbox_communitypersonal.yml; fi
...
question1 () {
  ...
  while read p; do
    ...
    touch /var/pgblitz/community.app
  #fi
...
boxuser=$(cat /var/pgblitz/boxcommunity.user)
boxbranch=$(cat /var/pgblitz/boxcommunity.branch)
...
EOF
        ...
        echo "$boxuser" > /var/pgblitz/boxcommunity.user
        echo "$boxbranch" > /var/pgblitz/boxcommunity.branch
        pinterface ;;
    2 )
...
case $typed in
    1 )
        ...
        question1 ;;
    2 )
        variable /var/pgblitz/boxcommunity.user "NOT-SET"
        variable /var/pgblitz/boxcommunity.branch "NOT-SET"
        pinterface ;;
    z )
        exit ;;
    Z )
