...
EOF
  ...
  if [ "$typed" == "1" ]; then echo public > /var/pgblitz/plex.extension && question2;
elif [ "$typed" == "2" ]; then echo plexpass > /var/pgblitz/plex.extension && question2;
else badinput; fi
}
...
EOF
  ...
  read -p 'Type a Number | Press [ENTER]: ' typed < /dev/tty
  echo ""
  if [ "$typed" == "1" ]; then echo remote > /var/pgblitz/plex.server && question3;
elif [ "$typed" == "2" ]; then echo local > /var/pgblitz/plex.server;
...
EOF
  ...
  echo $typed > /var/pgblitz/plex.claim && break=on;
}
